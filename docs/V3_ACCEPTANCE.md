...
| 24 | Project writer exclusion works. | PARTIAL | Shared source lock excludes writers during prepared/accepted ref transactions and after fetch-child death while transfer descendants remain alive. Different stores do not bypass it. Cross-source/abrupt-parent concurrency remains pending. |
| 25 | Crash/interruption tests pass. | PARTIAL | Prior boundaries/ref transactions plus real active loose-object arrival SIGINT/SIGTERM, fetch-child death and timeout pass. In-command add/commit, large-pack paths and abrupt-parent descendants remain pending. |
...
| 40 | Canonical deterministic test command passes. | COMPLETE | 292 tests pass (287 inherited unchanged + five active-transfer tests); exact final commands/output in progress. |
| 41 | Git publication of CGC V3 itself is verified. | COMPLETE | Cancellation/concurrency implementation 3721dd022d14b0470dbde429483312f9cc6314be pushed normally; local/tracking/live main equality and clean tree verified 2026-09-23. Evidence follow-up HEAD checked after publication; final report carries its resolved SHA. |
