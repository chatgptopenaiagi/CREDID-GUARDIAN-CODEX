...
from .daemon import run, refresh_once
from .engine import StateError, status, utcnow, PolicyConfig, DEFAULT_POLICY, MAX_AGE, SCHEMA_VERSION, validate_max_age, selection
from .quota import _validate_timeout
...
def main(argv=None):
    ...
    for name in ('status', 'refresh', 'daemon'):
    ...
    try:
        ...
        if args.command == 'status' and not args.cache_dir.exists():
            ...
            with Cache(args.cache_dir, create=args.command != 'status') as cache:
                if args.command != 'status':
                    stop = threading.Event()
                    old = {s: signal.signal(s, lambda *_: stop.set()) for s in (signal.SIGINT, signal.SIGTERM)}
                    try:
                        if args.command == 'refresh':
                            state = refresh_once(cache, buckets=args.bucket, max_age=args.max_age,
