...
POLICY_VERSION = 'cgc-default-v2'
SCHEMA_VERSION = 'cgc-state-v2'
MAX_AGE = 900
DIRECTIVES = {
...
def classify(value):
    ...
    return 'GREEN' if value > 20 else 'AMBER' if value > 10 else 'RED' if value > 5 else 'EMERGENCY'
...
def policy(obs, buckets):
    ...
    state = classify(minimum) if minimum is not None else None
    return {'policy_version': POLICY_VERSION, 'selected_buckets': buckets,
...
def empty_state(buckets, max_age=MAX_AGE):
    ...
    return {'schema_version': SCHEMA_VERSION, 'generation': 0,
            ...
            'policy': policy(None, buckets), 'last_refresh_attempt_at': None,
...
def validate_state(state):
    try:
        if not isinstance(state, dict) or state.get('schema_version') != SCHEMA_VERSION:
            raise StateError()
        template = empty_state(state['policy']['selected_buckets'], state['max_age_seconds'])
        ...
        if json.dumps(state['policy'], sort_keys=True, allow_nan=False) != json.dumps(policy(obs, state['policy']['selected_buckets']), sort_keys=True, allow_nan=False):
            raise StateError()
